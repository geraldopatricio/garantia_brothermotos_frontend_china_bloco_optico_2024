...
// Adicionamos 'viewAtiva' nas props para receber do componente pai qual tela está aberta
defineProps(['recolhida', 'viewAtiva'])
...
<template>
  ...
  >
    ...
    <nav class="flex-1 px-3 space-y-1 overflow-y-auto">
      <div>
        <p v-if="!recolhida" class="text-[10px] font-bold text-gray-400 uppercase px-3 py-2 tracking-widest text-left">Menu</p>
        
        <!-- Botão Dashboard -->
        <button @click="$emit('changeView', 'dashboard')" 
          :class="[
            baseBtn, 
            viewAtiva === 'dashboard' ? btnActive : btnNormal, 
            recolhida ? 'lg:justify-center px-3 lg:px-0' : 'px-3 justify-start'
          ]">
        ...
        <button @click="$emit('changeView', 'garantias')" 
          :class="[
            baseBtn, 
            viewAtiva === 'garantias' ? btnActive : btnNormal, 
            recolhida ? 'lg:justify-center px-3 lg:px-0' : 'px-3 justify-start'
          ]">
      ...
      <!-- Outros menus (mantidos como Normal pois parecem ser apenas placeholders) -->
      <div class="pt-4">
        <p v-if="!recolhida" class="text-[10px] font-bold text-gray-400 uppercase px-3 py-2 tracking-widest text-left">Ferramentas</p>
      ...
      <div class="pt-4">
        <p v-if="!recolhida" class="text-[10px] font-bold text-gray-400 uppercase px-3 py-2 tracking-widest text-left">Configurações</p>
        <button :class="[baseBtn, btnNormal, recolhida ? 'lg:justify-center px-3 lg:px-0' : 'px-3 justify-start']">
